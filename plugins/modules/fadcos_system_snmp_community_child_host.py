...
def add_obj(module, connection):
    ...
    url = obj_url + '?pkey=' + module.params['snmp_id']
...
def edit_obj(module, payload, connection):
    ...
    if pkey and mkey:
        ...
        module_err_exit(module, 'edit action needs the parameter \'snmp_id\' and \'id\' not to be empty.')
    if is_vdom_enable(connection) and module.params['vdom']:
        vdom = module.params['vdom']
        if pkey or mkey:
            url += '&vdom=' + vdom
...
def get_obj(module, connection):
    snmp_id = module.params['snmp_id']
    id = module.params['id']
    vdom = module.params['vdom']
    payload = {}
    if snmp_id:
        url = obj_url + '?pkey=' + snmp_id
        if id:
            url += '&mkey=' + id
    else:
        url = obj_url
